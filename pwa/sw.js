const CACHE='fourth-wall-v13',SHELL=['./','./index.html','./styles.css','./editor.css','./calendar.css','./app.js','./manifest.json','./icon.svg'];
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(caches.open(CACHE).then(async cache => {
    try {
      ...
      return (await cache.match(url.href)) || caches.match('./');
    }
  }));
});
