...
async function hmacHex(secret,value){const key=await crypto.subtle.importKey("raw",encoder.encode(secret),{name:"HMAC",hash:"SHA-256"},false,["sign"]);return toHex(await crypto.subtle.sign("HMAC",key,encoder.encode(value)))}
async function razorpayLink(env,invoice,client){
  ...
  const response=await fetch("https://api.razorpay.com/v1/payment_links",{method:"POST",headers:{authorization:`Basic ${btoa(`${env.RAZORPAY_KEY_ID}:${env.RAZORPAY_KEY_SECRET}`)}`,"content-type":"application/json"},body:JSON.stringify({amount:invoice.total,currency:invoice.currency,reference_id:invoice.id,description:`${invoice.invoice_number} · ${invoice.project_name}`,customer:{name:client?.display_name||"Client",email:client?.email},notify:{email:true,sms:false},reminder_enable:true,callback_url:env.PUBLIC_APP_URL||"https://fourthwall.pages.dev/",callback_method:"get",notes:{invoice_id:invoice.id,project_id:invoice.project_id}})});
  if(!response.ok)throw Error(`Razorpay ${response.status}: ${(await response.text()).slice(0,300)}`);return response.json();
}
async function signedStorageUrl(env,path){const response=await fetch(`${env.SUPABASE_URL}/storage/v1/object/sign/${env.SUPABASE_STORAGE_BUCKET}/${path}`,{method:"POST",headers:{...storageHeaders(env),"content-type":"application/json"},body:JSON.stringify({expiresIn:3600})});if(!response.ok)throw Error("Could not prepare contract file");const value=await response.json();return `${env.SUPABASE_URL}/storage/v1${value.signedURL}`}
async function dropboxSignRequest(env,proposal,client,fileUrl){
  ...
  const response=await fetch("https://api.hellosign.com/v3/signature_request/send",{method:"POST",headers:{authorization:`Basic ${btoa(`${env.DROPBOX_SIGN_API_KEY}:`)}`,"content-type":"application/json"},body:JSON.stringify({file_urls:[fileUrl],title:proposal.title,subject:`Signature requested: ${proposal.title}`,message:"Please review and sign your Fourth Wall proposal.",signers:[{email_address:client.email,name:client.display_name,order:0}],metadata:{proposal_id:proposal.id,project_id:proposal.project_id},test_mode:env.DROPBOX_SIGN_TEST_MODE!=="false"})});
  if(!response.ok)throw Error(`Dropbox Sign ${response.status}: ${(await response.text()).slice(0,300)}`);return response.json();
}
async function googleAccessToken(env){
  if(!env.GOOGLE_SERVICE_ACCOUNT_EMAIL||!env.GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY)throw Error("Google Calendar is not configured");
  const now=Math.floor(Date.now()/1000),header=base64url(encoder.encode(JSON.stringify({alg:"RS256",typ:"JWT"}))),claims=base64url(encoder.encode(JSON.stringify({iss:env.GOOGLE_SERVICE_ACCOUNT_EMAIL,scope:"https://www.googleapis.com/auth/calendar",aud:"https://oauth2.googleapis.com/token",iat:now,exp:now+3600}))),pem=env.GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY.replace(/\\n/g,"\n").replace(/-----[^-]+-----/g,"").replace(/\s/g,"");
  const bytes=Uint8Array.from(atob(pem),c=>c.charCodeAt(0)),key=await crypto.subtle.importKey("pkcs8",bytes,{name:"RSASSA-PKCS1-v1_5",hash:"SHA-256"},false,["sign"]),signature=base64url(await crypto.subtle.sign("RSASSA-PKCS1-v1_5",key,encoder.encode(`${header}.${claims}`)));
  const response=await fetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:new URLSearchParams({grant_type:"urn:ietf:params:oauth:grant-type:jwt-bearer",assertion:`${header}.${claims}.${signature}`})});if(!response.ok)throw Error("Google Calendar authorization failed");return (await response.json()).access_token;
}
async function googleCalendarEvent(env,meeting,attendees){const token=await googleAccessToken(env),calendar=encodeURIComponent(env.GOOGLE_CALENDAR_ID||"primary"),end=new Date(new Date(meeting.starts_at).getTime()+meeting.duration_minutes*60000).toISOString(),response=await fetch(`https://www.googleapis.com/calendar/v3/calendars/${calendar}/events?conferenceDataVersion=1&sendUpdates=all`,{method:"POST",headers:{authorization:`Bearer ${token}`,"content-type":"application/json"},body:JSON.stringify({summary:meeting.title,description:"The Fourth Wall project meeting",start:{dateTime:new Date(meeting.starts_at).toISOString()},end:{dateTime:end},attendees:attendees.map(email=>({email})),conferenceData:{createRequest:{requestId:meeting.id,conferenceSolutionKey:{type:"hangoutsMeet"}}},reminders:{useDefault:false,overrides:[{method:"email",minutes:1440},{method:"popup",minutes:30}]}})});if(!response.ok)throw Error(`Google Calendar ${response.status}: ${(await response.text()).slice(0,300)}`);return response.json()}
...
export default {
  async fetch(request, env) {
    ...
    if(url.pathname==="/api/webhooks/razorpay"&&request.method==="POST"){
      if(!env.RAZORPAY_WEBHOOK_SECRET)return deny("Webhook is not configured",503);const raw=await request.text(),actual=request.headers.get("x-razorpay-signature")||"",expected=await hmacHex(env.RAZORPAY_WEBHOOK_SECRET,raw);if(actual.length!==expected.length||!actual.split("").every((c,i)=>c===expected[i]))return deny("Invalid webhook signature",401);const event=JSON.parse(raw),link=event?.payload?.payment_link?.entity;if(event?.event==="payment_link.paid"&&link?.id){const invoice=await env.DB.prepare("select id,project_id from invoices where payment_link_id=?").bind(link.id).first();if(invoice){await env.DB.prepare("update invoices set status='paid',paid_at=current_timestamp,updated_at=current_timestamp where id=?").bind(invoice.id).run();await audit(env,null,invoice.project_id,"paid","invoice",invoice.id,{provider:"razorpay",paymentLinkId:link.id})}}return json({ok:true});
    }
    const commercialMatch=url.pathname.match(/^\/api\/projects\/([^/]+)\/commercial$/);
    if(commercialMatch&&request.method==="GET"){
      const user=await userFromRequest(request,env),projectId=commercialMatch[1];if(!user||!(await canAccessProject(env,user,projectId)))return deny("Project access required",403);const [invoices,proposals,meetings]=await Promise.all([env.DB.prepare("select * from invoices where project_id=? and (?=1 or status!='draft') order by created_at desc").bind(projectId,studio(user)?1:0).all(),env.DB.prepare("select * from proposals where project_id=? and (?=1 or status!='draft') order by created_at desc").bind(projectId,studio(user)?1:0).all(),env.DB.prepare("select * from meetings where project_id=? order by starts_at desc").bind(projectId).all()]);return json({invoices:invoices.results,proposals:proposals.results,meetings:meetings.results,providers:{razorpay:Boolean(env.RAZORPAY_KEY_ID),dropboxSign:Boolean(env.DROPBOX_SIGN_API_KEY),googleCalendar:Boolean(env.GOOGLE_SERVICE_ACCOUNT_EMAIL)}});
    }
    const invoicesMatch=url.pathname.match(/^\/api\/projects\/([^/]+)\/invoices$/);
    if(invoicesMatch&&request.method==="POST"){
      const user=await userFromRequest(request,env);if(!studio(user))return deny("Studio access required",403);const p=await body(request),projectId=invoicesMatch[1],source=p?.source==="tracked_time"?"tracked_time":"fixed";let items=Array.isArray(p?.items)?p.items.slice(0,30):[];if(source==="tracked_time"){const minutes=(await env.DB.prepare("select coalesce(sum(minutes),0) total from time_entries where project_id=?").bind(projectId).first())?.total||0,rate=Math.max(0,Math.round(Number(p?.hourlyRate)||0));items=[{description:`Tracked studio time (${Math.round(minutes/6)/10} hours)`,quantity:1,unitAmount:Math.round(minutes*rate/60)}]}items=items.map((x,i)=>({description:String(x.description||"Service").trim().slice(0,300),quantity:Math.max(1,Math.round(Number(x.quantity)||1)),unitAmount:Math.max(0,Math.round(Number(x.unitAmount)||0)),sortOrder:i})).filter(x=>x.unitAmount>0);if(!items.length)return deny("Add at least one invoice item",400);const subtotal=items.reduce((sum,x)=>sum+x.quantity*x.unitAmount,0),tax=Math.max(0,Math.round(Number(p?.tax)||0)),total=subtotal+tax,id=crypto.randomUUID(),number=`TFW-${new Date().getUTCFullYear()}-${id.slice(0,6).toUpperCase()}`;await env.DB.batch([env.DB.prepare("insert into invoices(id,project_id,invoice_number,currency,subtotal,tax,total,status,source,due_date,created_by) values(?,?,?,?,?,?,?,?,?,?,?)").bind(id,projectId,number,String(p?.currency||"INR").slice(0,3),subtotal,tax,total,p?.sendNow?"sent":"draft",source,p?.dueDate||null,user.id),...items.map(x=>env.DB.prepare("insert into invoice_items(id,invoice_id,description,quantity,unit_amount,sort_order) values(?,?,?,?,?,?)").bind(crypto.randomUUID(),id,x.description,x.quantity,x.unitAmount,x.sortOrder))]);await audit(env,user,projectId,"created","invoice",id,{number,total,source});return json({id,invoiceNumber:number,total},201);
    }
    const paymentMatch=url.pathname.match(/^\/api\/invoices\/([^/]+)\/payment-link$/);
    if(paymentMatch&&request.method==="POST"){
      const user=await userFromRequest(request,env);if(!user)return deny("Sign in required");const invoice=await env.DB.prepare("select i.*,p.name project_name from invoices i join projects p on p.id=i.project_id where i.id=?").bind(paymentMatch[1]).first();if(!invoice||!(await canAccessProject(env,user,invoice.project_id)))return deny("Invoice not found",404);if(invoice.status==="paid")return json({url:invoice.checkout_url,status:"paid"});const client=await env.DB.prepare("select u.email,u.display_name from project_members pm join users u on u.id=pm.user_id where pm.project_id=? and u.role='client' limit 1").bind(invoice.project_id).first();try{const link=await razorpayLink(env,invoice,client);await env.DB.prepare("update invoices set payment_provider='razorpay',payment_link_id=?,checkout_url=?,status='sent',updated_at=current_timestamp where id=?").bind(link.id,link.short_url,invoice.id).run();await audit(env,user,invoice.project_id,"payment_link_created","invoice",invoice.id,{provider:"razorpay"});return json({url:link.short_url,status:link.status})}catch(error){return deny(error.message,503)}
    }
    const proposalsMatch=url.pathname.match(/^\/api\/projects\/([^/]+)\/proposals$/);
    if(proposalsMatch&&request.method==="POST"){
      const user=await userFromRequest(request,env);if(!studio(user))return deny("Studio access required",403);const p=await body(request),title=String(p?.title||"").trim().slice(0,255),blocks=(Array.isArray(p?.blocks)?p.blocks:[]).slice(0,30).map((x,i)=>({title:String(x.title||"Service").slice(0,180),description:String(x.description||"").slice(0,3000),amount:Math.max(0,Math.round(Number(x.amount)||0)),sortOrder:i}));if(!title||!blocks.length)return deny("Proposal title and service blocks are required",400);if(p?.pdfFileId){const file=await env.DB.prepare("select 1 from project_files where id=? and project_id=? and mime_type='application/pdf'").bind(p.pdfFileId,proposalsMatch[1]).first();if(!file)return deny("Choose a project PDF",400)}const id=crypto.randomUUID(),total=blocks.reduce((n,x)=>n+x.amount,0);await env.DB.batch([env.DB.prepare("insert into proposals(id,project_id,title,introduction,currency,total,status,pdf_file_id,created_by) values(?,?,?,?,?,?,?,?,?)").bind(id,proposalsMatch[1],title,String(p?.introduction||"").slice(0,5000),String(p?.currency||"INR").slice(0,3),total,p?.sendNow?"sent":"draft",p?.pdfFileId||null,user.id),...blocks.map(x=>env.DB.prepare("insert into proposal_blocks(id,proposal_id,title,description,amount,sort_order) values(?,?,?,?,?,?)").bind(crypto.randomUUID(),id,x.title,x.description,x.amount,x.sortOrder))]);await audit(env,user,proposalsMatch[1],"created","proposal",id,{title,total});return json({id,total},201);
    }
    const signRequestMatch=url.pathname.match(/^\/api\/proposals\/([^/]+)\/send-signature$/);
    if(signRequestMatch&&request.method==="POST"){
      const user=await userFromRequest(request,env);if(!studio(user))return deny("Studio access required",403);const proposal=await env.DB.prepare("select pr.*,pf.storage_path from proposals pr left join project_files pf on pf.id=pr.pdf_file_id where pr.id=?").bind(signRequestMatch[1]).first();if(!proposal?.storage_path)return deny("Attach a proposal PDF first",400);const client=await env.DB.prepare("select u.email,u.display_name from project_members pm join users u on u.id=pm.user_id where pm.project_id=? and u.role='client' limit 1").bind(proposal.project_id).first();if(!client)return deny("Add a client to this project first",409);try{const sent=await dropboxSignRequest(env,proposal,client,await signedStorageUrl(env,proposal.storage_path)),requestId=sent.signature_request.signature_request_id;await env.DB.prepare("update proposals set signature_request_id=?,status='sent',updated_at=current_timestamp where id=?").bind(requestId,proposal.id).run();await audit(env,user,proposal.project_id,"sent_for_signature","proposal",proposal.id,{provider:"dropbox_sign",requestId});return json({requestId})}catch(error){return deny(error.message,503)}
    }
    const meetingsMatch=url.pathname.match(/^\/api\/projects\/([^/]+)\/meetings$/);
    if(meetingsMatch&&request.method==="POST"){
      const user=await userFromRequest(request,env),projectId=meetingsMatch[1];if(!user||!(await canAccessProject(env,user,projectId)))return deny("Project access required",403);const p=await body(request),startsAt=new Date(p?.startsAt),duration=Math.max(15,Math.min(240,Number(p?.durationMinutes)||30)),title=String(p?.title||"Project check-in").trim().slice(0,180);if(Number.isNaN(startsAt.valueOf())||startsAt.getTime()<Date.now())return deny("Choose a future meeting time",400);const id=crypto.randomUUID(),meeting={id,project_id:projectId,title,starts_at:startsAt.toISOString(),duration_minutes:duration};const attendees=(await env.DB.prepare("select distinct u.email from project_members pm join users u on u.id=pm.user_id where pm.project_id=?").bind(projectId).all()).results.map(x=>x.email);try{const event=await googleCalendarEvent(env,meeting,attendees),joinUrl=event.hangoutLink||event.htmlLink;await env.DB.prepare("insert into meetings(id,project_id,title,starts_at,duration_minutes,status,meeting_provider,join_url,calendar_event_id,booked_by) values(?,?,?,?,?,'confirmed','google_calendar',?,?,?)").bind(id,projectId,title,meeting.starts_at,duration,joinUrl,event.id,user.id).run();await audit(env,user,projectId,"booked","meeting",id,{provider:"google_calendar",startsAt:meeting.starts_at});return json({id,joinUrl,status:"confirmed"},201)}catch(error){return deny(error.message,503)}
    }
